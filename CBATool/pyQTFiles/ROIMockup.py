...
# Form implementation generated from reading ui file 'Chima Ugwuegbulam made the foundation of this file.'
#
...
class Ui_Form(object):
    ...
    def setupUi(self, Form):
        ...
        self.ordersSpinBox.setMaximum(1000)
        ...
        self.ordersLabel.setSizePolicy(sizePolicy)
        ...
        self.ordersSpinBox.setAlignment(QtCore.Qt.AlignLeft)
        self.yearsSpinBox.setAlignment(QtCore.Qt.AlignLeft)
        self.sparesSpinBox.setAlignment(QtCore.Qt.AlignLeft)
        ...
        # sets everything for row 3 (EXCLUSIVELY THE TABLEVIEW)
    ...
    def runClicked(self, Form):
        ...
        if totalOrders >= year_range:
            ...
            step_2_o = orders_value - step_1_o
            step_2_s = spare_value - step_1_s

            step_3_o = step_2_o / year_range
            step_3_s = step_2_s / year_range

            row_list = [step_3_o] * year_range
            spares_list = [step_3_s] * year_range
            if step_1_o:
                for x in range(step_1_o):
                    row_list[-(x+1)] = step_3_o + 1

            if step_1_s:
                for x in range(step_1_s):
                    spares_list[-(x+1)] = step_3_s + 1
    ...
    def pdfClicked(self, Form):
        print(self.df)
        pdfList = self.df.values.tolist()
        pdfList_df = pd.DataFrame(pdfList)
        pdfList_df= pdfList_df.transpose()
        new_list = []
        for x in range(pdfList_df.shape[0]):
            new_list.append(pdfList_df.loc[x, :].values.tolist())
        new_df = pd.DataFrame(new_list)

        new_df = pd.DataFrame(new_list, columns=['Baseline', 'Percent Decrease', 'No. of Orders',
           'No. of Spares'])
        print(new_df)
        totalList = self.total_values_list
        pdf_report(new_df, totalList)
